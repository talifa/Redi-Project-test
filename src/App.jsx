...
const App = () => {
  ...
  useEffect(() => {
      ...
      console.log(movieFav);
       if (movieFav && movieFav.length !==0) { setFavour(movieFav)}; 
  }, []);

  useEffect(() => {
    const movieDate = JSON.parse(
      localStorage.getItem('react-date-favorites')
    );
    if ( movieDate && movieDate.length !==0) { setFilmplaned(movieDate)};
...
const planFavourMovies = () => {
  ...
  const removeFavourMovie = ( movie) =>{
    const newFavourList = favour.filter(
  ...
  const addDate= (date) => {
    const newDateList = [...selectedDate, date.toDateString()].sort((a, b) => new Date(a) - new Date(b));;
  ...
  return (
   ...
   <Wrapper>
    ...
    <Header>
       ...
       <SearchQuery searchValue={searchValue} setSearchValue={setSearchValue}  />
    </Header>

    <MoviePlanedList filmplaned = {filmplaned}  handlePage={ handlePage}  button ="make my watch list" planFavorMovies={planFavourMovies}/>
    
    <MovieList movies = {movies} button ="add to favourites" handleFavourClick={addFavourMovie} />  
 
   <MovieHeading heading ="Favourites"/> 
  
   <MovieList movies = {favour} button ="delete" handleFavourClick={removeFavourMovie}/>  
